...
PRIVATE int fit_algorithm = FIRST_FIT_ALG; /* first fit is the standard algorithm */
 ...
 */
  ...
  register struct hole *hp, *prev_ptr, *biggest_hole, *biggest_hole_prev;
...
switch(fit_algorithm)
{
  ...
  case WORST_FIT_ALG:
    do {
      hp = hole_head;
      biggest_hole = hp;
      biggest_hole_prev = NIL_HOLE;
      while (hp != NIL_HOLE && hp->h_base < swap_base) {
        if(biggest_hole->h_len < hp->h_len) {
          biggest_hole_prev = prev_ptr;
          biggest_hole = hp;
        }
        prev_ptr = hp;
        hp = hp->h_next;
      ...
      if(biggest_hole->h_len >= clicks) {
        ...
        /* Delete the hole if used up completely. */
        if (biggest_hole->h_len == 0) {
          del_slot(biggest_hole_prev, biggest_hole);
        }
...
{
  /* (size, addr) * number of holes + ending 0 */
  phys_clicks buffer[2*NR_HOLES + 1];
  phys_clicks count_max_number;
  phys_clicks counter;
  struct hole *hole_ptr;

  /* mm.m1_i1 - number of bytes */
  count_max_number = mm_in.m1_i1 / sizeof(phys_clicks);
  counter = 0;
  hole_ptr = hole_head;
  while(counter + 2 < count_max_number && hole_ptr != NIL_HOLE && hole_ptr->h_base < swap_base)
  {
    buffer[counter++] = hole_ptr->h_len;
    buffer[counter++] = hole_ptr->h_base;
    hole_ptr = hole_ptr->h_next;
  }
  buffer[counter] = 0;
  ...
  */
  ...
  sys_copy(MM_PROC_NR, 
           ...
           who, 
           D, 
           (phys_clicks) mm_in.m1_p1, 
           (phys_clicks) mm_in.m1_i1);
...
PUBLIC int do_worst_fit(void)
{
  ...
  {
    case FIRST_FIT_ALG:
      fit_algorithm = FIRST_FIT_ALG;
    break;
    case WORST_FIT_ALG:
      fit_algorithm = WORST_FIT_ALG;
    break;
  }
  return 0;
}
