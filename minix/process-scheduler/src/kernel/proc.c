 ...
 */
  ...
  */
  ...
  if(rdy_head[USER_Q] -> quants_left > 1) {
    (rdy_head[USER_Q] -> quants_left)--;
    pick_proc();
    return;
  } 
  /* if there is only one quant left, 
  sched should pick this process and move it to the end of the queue
  and reset quants_left */
  rdy_tail[USER_Q] -> p_nextready = rdy_head[USER_Q];
  rdy_tail[USER_Q] = rdy_head[USER_Q];
  rdy_head[USER_Q] = rdy_head[USER_Q] -> p_nextready;
  rdy_tail[USER_Q] -> p_nextready = NIL_PROC;
  rdy_tail[USER_Q] -> quants_left = rdy_tail[USER_Q] -> pri;
  pick_proc();
